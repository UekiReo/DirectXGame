...
HRESULT D3D12Manager::CreateCommandQueue() {
	...
	hr = device_->CreateCommandQueue(
	    &command_queue_desc, IID_PPV_ARGS(command_queue_.GetAddressOf()));
	if (FAILED(hr)) {
		return hr;
	}
...
HRESULT D3D12Manager::CreateSwapChain() {
	...
	hr = factory_->CreateSwapChain(
	    command_queue_.Get(), &swap_chain_desc, swap_chain.GetAddressOf());
	if (FAILED(hr)) {
		return hr;
	}
...
HRESULT D3D12Manager::CreateDepthStencilBuffer() {
	...
	hr = device_->CreateCommittedResource(
	    &heap_properties, D3D12_HEAP_FLAG_NONE, &resource_desc, D3D12_RESOURCE_STATE_DEPTH_WRITE,
...
HRESULT D3D12Manager::CreateRootSignature() {
	...
	hr = D3D12SerializeRootSignature(
	    &root_signature_desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, nullptr);
	if (FAILED(hr)) {
		return hr;
	}
	hr = device_->CreateRootSignature(
	    0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&root_sugnature_));
...
HRESULT D3D12Manager::CreatePipelineStateObject() {
	...
	hr = D3DCompileFromFile(
	    L"Shaders.hlsl", nullptr, nullptr, "VSMain", "vs_5_0", compile_flag, 0,
	    vertex_shader.GetAddressOf(), nullptr);
	if (FAILED(hr)) {
		return hr;
	}

	hr = D3DCompileFromFile(
	    L"Shaders.hlsl", nullptr, nullptr, "PSMain", "ps_5_0", compile_flag, 0, &pixel_shader,
	    nullptr);
	if (FAILED(hr)) {
		return hr;
	}
	...
	pipeline_state_desc.RasterizerState.ConservativeRaster =
	...
	for (int i = 0; i < _countof(pipeline_state_desc.BlendState.RenderTarget); ++i) {
		...
		pipeline_state_desc.BlendState.RenderTarget[i].RenderTargetWriteMask =
...
HRESULT D3D12Manager::SetResourceBarrier(
